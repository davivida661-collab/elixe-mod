...
public class AutoSoup extends Module {
	...
	public AutoSoup() {
		...
		moduleOptions.add(drinkRandomOption);
	...
	boolean drinkRandom;
	ModuleBoolean drinkRandomOption = new ModuleBoolean("drink random", false) {
		public void valueChanged() {
			drinkRandom = (boolean) this.getValue();
		}
	};
								...
								if (recraftableItems[i]) {
									for (int j = 0; j < combinationItems[i].length; j++) {
										if (combinationItems[i][j] instanceof Block) { // block
											InventoryItem block = InventoryItem.findBlockHighest(9, 45,
													(Block) combinationItems[i][j], mc);
											if (block.getSlot() != -1) {
											...
											// spoiler: nunca refatorei
											InventoryItem item = InventoryItem.findItemHighest(9, 45,
													(Item) combinationItems[i][j], 3, mc);
											if (item.getSlot() != -1) {
								...
								for (int j = 0; j < bakedCombinations.length; j++) {
									...
									for (int k = 0; k < bakedCombinations[j].length; k++) {
										...
										if (bakedCombinations[j].length + i > sortedItems.size()) {
											...
											// se index atual do loop de itens + index do loop da combinacao nao sao da
											// combinacao
	...
	private void makeRecraftStep() {
		if (recraftTimer.hasTimePassed(recraftDelay)) {
			...
			if (newRecraft) {
				...
				case 2: // coloca o item na crafting
				...
				case 8: // shift na crafting pra fazer tudo de sopa
					mc.playerController.windowClick(0, 0, 0, 1, mc.thePlayer);
					break;

				case 9: // shift na crafting pra tirar os itens
	...
	private Listener<OnKeybindActionEvent> onKeybindActionEvent = new Listener<>(e -> {
		if (waitForUseItem) {
			...
			if (e.getKey() == useItem) {
				if (!((elixe.mixin.AccessorPlayerControllerMP) mc.playerController).isHittingBlock()) {
	...
	private void makeAutoSoupStep() {
		if (drinkTimer.hasTimePassed(drinkDelay)) {
			switch (autoSoupStep) {
			case 1: // toma a sopa
	...
	private boolean shouldAutoSoup(boolean first) {
		if (healthToSoup > mc.thePlayer.getHealth()) { // health menor do que setado
			// pega uma sopa aleatoria da hotbar (se ligado) em vez de sempre a primeira
			soupInHotbar = drinkRandom ? InventoryItem.findItemRandomized(36, 45, Items.mushroom_stew, mc)
					: InventoryItem.findItem(36, 45, Items.mushroom_stew, mc);
