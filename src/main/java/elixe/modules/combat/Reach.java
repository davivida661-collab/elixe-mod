...
import elixe.mixin.AccessorEntityRenderer;
...
public class Reach extends Module {
	...
	private Listener<OnGetMouseOverEvent> onGetMouseOverEvent = new Listener<>(e -> {
		...
		AccessorEntityRenderer accEr = (AccessorEntityRenderer) entityRenderer;
		...
		if (entity != null && this.mc.theWorld != null) {
			...
			for (int j = 0; j < list.size(); ++j) {
				...
				if (axisalignedbb.isVecInside(vec3)) { // estamos dentro do mob
					if (d2 >= 0.0D) {
						accEr.setPointedEntity(entity1);
			...
			if (accEr.getPointedEntity() != null) {
				if (activationChance > r.nextFloat() * 100f) {
					if (yCheck) {
						if (isEntityInDifferentY(accEr.getPointedEntity(), entity)) {
							customReach = 3f;
						}
					}
					if (ignoreNaked) {
						if (EntityUtils.isNaked((EntityLivingBase) accEr.getPointedEntity())) {
							customReach = 3f;
						}
					}
				} else {
			...
			if (accEr.getPointedEntity() != null && flag && vec3.distanceTo(vec33) > customReach) {
				accEr.setPointedEntity(null);
			...
			if (accEr.getPointedEntity() != null && (d2 < d1 || this.mc.objectMouseOver == null)) {
				this.mc.objectMouseOver = new MovingObjectPosition(accEr.getPointedEntity(), vec33);

				if (accEr.getPointedEntity() instanceof EntityLivingBase
						|| accEr.getPointedEntity() instanceof EntityItemFrame) {
					this.mc.pointedEntity = accEr.getPointedEntity();
				}
			}
