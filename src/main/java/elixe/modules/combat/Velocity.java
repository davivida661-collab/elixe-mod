...
public class Velocity extends Module {
	...
	// 0 = simple (scale the incoming velocity packet), 1 = legit (scale real motion on
	// the knockback tick), 2 = jump (re-jump into the knockback to reset it)
	int mode;
	ModuleArray modeOption = new ModuleArray("mode", 0, new String[] { "simple", "legit", "jump" }) {
	...
	// simple mode: scale the server velocity packet before it is applied
	@EventHandler
	private Listener<OnPacketReceiveEvent> onPacketReceiveEvent = new Listener<>(e -> {
		if (mode != 0 || mc.thePlayer == null) {
			return;
		}
		if (e.getPacket() instanceof S12PacketEntityVelocity) {
			S12PacketEntityVelocity vel = (S12PacketEntityVelocity) e.getPacket();
			if (vel.getEntityID() == mc.thePlayer.getEntityId()) {
				if (shouldModifyVelocity() && activationChance > r.nextFloat() * 100f) {
					((elixe.mixin.AccessorS12PacketEntityVelocity) vel).setMotionX((int) (vel.getMotionX() * horizontalMultiplier));
					((elixe.mixin.AccessorS12PacketEntityVelocity) vel).setMotionZ((int) (vel.getMotionZ() * horizontalMultiplier));
					((elixe.mixin.AccessorS12PacketEntityVelocity) vel).setMotionY((int) (vel.getMotionY() * verticalMultiplier));
					e.setPacket(vel);
				}
			}
		}
	...
	// legit / jump modes: act on the single tick the knockback actually lands
	@EventHandler
	private Listener<OnLivingUpdateEvent> onLivingUpdateEvent = new Listener<>(e -> {
		...
		boolean knockbackTick = mc.thePlayer.hurtResistantTime != 0
				&& mc.thePlayer.maxHurtResistantTime == mc.thePlayer.hurtResistantTime;
		if (!knockbackTick || !shouldModifyVelocity() || activationChance <= r.nextFloat() * 100f) {
			return;
		}

		if (mode == 1) {
			// legit: reduce the real motion vector
			...
			// jump-reset
			if (mc.thePlayer.onGround && conditionals.isSprinting()) {
