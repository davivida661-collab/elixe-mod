...
public class Chams extends Module {
	...
	// substitui o campo custom Entity.chamsAllowed do MCP-919
	java.util.HashSet<Entity> chamsAllowedSet = new java.util.HashSet<Entity>();

	@EventHandler
	private Listener<OnTickEvent> onTickEvent = new Listener<>(e -> {
		if (mc.theWorld == null)
			return;
		chamsAllowedSet.clear();
		for (Entity ent : mc.theWorld.loadedEntityList) {
			// guard 'rendered' removido na migração
			if (ent == mc.thePlayer) {
				continue;
			}
			boolean elixe$chams = ((ent instanceof EntityPlayer && allowedEntities[0]) || (ent instanceof EntityAnimal && allowedEntities[1])
					|| ((ent instanceof EntityMob || ent instanceof EntitySlime) && allowedEntities[2])
					|| (ent instanceof EntityVillager && allowedEntities[3]));
			if (elixe$chams) chamsAllowedSet.add(ent); else chamsAllowedSet.remove(ent);
	...
	private Listener<OnBrightnessEntityEvent> onBrightnessEntityEvent = new Listener<>(e -> {
		if (addLight && chamsAllowedSet.contains(e.getEnt())) {
			e.setLight(13631696);
		}
	});
	...
	private Listener<OnRenderEntityEvent> onRenderEntityEvent = new Listener<>(e -> {
		if (chamsAllowedSet.contains(e.getEntity())) {
			switch (e.getState()) {
			case 0: // pre all
	...
	private void setColor(RendererLivingEntity<EntityLivingBase> renderer, float r, float g, float b) {
		elixe.mixin.AccessorRendererLivingEntity arl = (elixe.mixin.AccessorRendererLivingEntity) renderer;
		...
		arl.getBrightnessBuffer().put(r);
		arl.getBrightnessBuffer().put(g);
		arl.getBrightnessBuffer().put(b);
		arl.getBrightnessBuffer().put(1F);
