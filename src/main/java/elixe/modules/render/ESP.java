...
public class ESP extends Module {
	...
	// substitui o campo custom Entity.espAllowed do MCP-919
	java.util.HashSet<Entity> espAllowedSet = new java.util.HashSet<Entity>();

	@EventHandler
	private Listener<OnRenderNameEvent> onRenderNameEvent = new Listener<>(e -> {
		if (espAllowedSet.contains(e.getEntity())) {
			e.cancel();
		}
	});
	...
	private Listener<OnTickEvent> onTickEvent = new Listener<>(e -> {
		...
		for (Entity ent : mc.theWorld.loadedEntityList) {
			// guard 'rendered' removido na migração; o render já faz frustum culling
			if (ent == mc.thePlayer) {
				continue;
			}
			if ((ent instanceof EntityPlayer && allowedEntities[0])
				...
				espAllowedSet.add(ent);
				filteredEntities.add(ent);
			} else {
				espAllowedSet.remove(ent);
			}
	...
	private void drawName(float minX, float maxX, float minY, float maxY, EntityLivingBase entLiving) {
		...
		switch (nameLocation) {
		case 0: // up
			elixe.utils.misc.FontUtil.drawStringWithShadow(mc.fontRendererObj, entName, minX + espWid - textWidth, minY - 10 - spacingYUp, 0xFFFFFFFF);
			spacingYUp += 10;
			break;
		case 1: // down
			elixe.utils.misc.FontUtil.drawStringWithShadow(mc.fontRendererObj, entName, minX + espWid - textWidth, maxY + 2 + spacingYDown, 0xFFFFFFFF);
			spacingYDown += 10;
			break;
		}
	...
	private void drawItemEnch(float minX, float maxX, float minY, float maxY, EntityLivingBase entLiving) {
		...
		if (entItem != null) {
			...
			case 0: // up
				elixe.utils.misc.FontUtil.drawStringWithShadow(mc.fontRendererObj, entItemEnch, minX + espWid - textWidth, minY - 10 - spacingYUp, 0xFFFFFFFF);
				spacingYUp += 10;
				break;
			case 1: // down
				elixe.utils.misc.FontUtil.drawStringWithShadow(mc.fontRendererObj, entItemEnch, minX + espWid - textWidth, maxY + 2 + spacingYDown, 0xFFFFFFFF);
				spacingYDown += 10;
				break;
			}
	...
	private void drawItemName(float minX, float maxX, float minY, float maxY, EntityLivingBase entLiving) {
		...
		if (entItem != null) {
			...
			case 0: // up
				elixe.utils.misc.FontUtil.drawStringWithShadow(mc.fontRendererObj, entItemName, minX + espWid - textWidth, minY - 10 - spacingYUp, 0xFFFFFFFF);
				spacingYUp += 10;
				break;
			case 1: // down
				elixe.utils.misc.FontUtil.drawStringWithShadow(mc.fontRendererObj, entItemName, minX + espWid - textWidth, maxY + 2 + spacingYDown, 0xFFFFFFFF);
				spacingYDown += 10;
				break;
			}
	...
	private void drawItemIcon(float minX, float maxX, float minY, float maxY, EntityLivingBase entLiving) {
		...
		if (item != null) {
			...
			case 0: // up
				renderItem.renderItemIntoGUI(item, (int) (minX + espWid - 8), (int) (minY - 16 - spacingYUp));
				break;

			case 1: // down
				renderItem.renderItemIntoGUI(item, (int) (minX + espWid - 8), (int) (maxY + 2 + spacingYDown));
				break;
			}
	...
	private float[] calculatePoints(Entity ent, float tick) {
		AxisAlignedBB bb = ent.getEntityBoundingBox().offset(-ent.posX, -ent.posY, -ent.posZ)
				...
				.offset(-((elixe.mixin.AccessorRenderManager) renderManager).getRenderPosX(), -((elixe.mixin.AccessorRenderManager) renderManager).getRenderPosY(), -((elixe.mixin.AccessorRenderManager) renderManager).getRenderPosZ());
