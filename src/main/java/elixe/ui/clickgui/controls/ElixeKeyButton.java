...
public class ElixeKeyButton extends ElixeButtonBase {
	...
	public void drawText(int mouseX, int mouseY) {
		...
		elixe.utils.misc.FontUtil.drawString(fontrenderer, keyName, x + width - boxWidth / 2 - keyNameTextWidth, controlMiddle - fontrenderer.FONT_HEIGHT / 2,
				0.07f, 1f); // 20
	}
	...
	public void drawButton(int mouseX, int mouseY) {
		if (waiting) {
			GUIUtils.drawPolygon(keyBackground, ElixeTheme.ACCENT[0], ElixeTheme.ACCENT[1], ElixeTheme.ACCENT[2], 1f);
			GL11.glColor4f(ElixeTheme.ACCENT[0], ElixeTheme.ACCENT[1], ElixeTheme.ACCENT[2], 1f);
		} else {
			GUIUtils.drawPolygon(keyBackground, 0.47f, 1f);
			GL11.glColor4f(0.47f, 0.47f, 0.47f, 1f);
		}
